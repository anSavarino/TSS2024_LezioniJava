...
public class RegioniDemo {
	...
	public static void main(String[] args) {
		...
		Regione lombardia = new Regione("Lombardia", 10_000_000, Set.of(
				new Provincia(1, new Regione(), "Milano", 500_000, 500_000, 1_000_000),
				new Provincia(2, new Regione(), "Como", 100_000, 100_000, 200_000),
				new Provincia(3, new Regione(), "Varese", 200_000, 200_000, 400_000)
				));
		...
		ctr.getListRegioni().forEach(r -> System.out.println(r));
		
		ctr
			.getRegioniByPopolazioneBetween(2_000_000, 6_000_000)
			.forEach(System.out::println);
		
		List<Set<Provincia>> list = ctr.getListRegioni().stream()
				.map(r -> r.getProvince())
				.toList();
		List<List<Provincia>> list2 = list.stream()
				.map(pp -> pp.stream().toList())
				.toList();
